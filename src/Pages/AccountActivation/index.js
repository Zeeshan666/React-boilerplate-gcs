...
import { ACCOUNT_ACTIVATION_VERIFY, USER_PASSWORD_REGEX } from '../../Common/index';
...
const Index = () => {
  ...
  useEffect(() => {
    document.title = "BidSushi | Activate Account";
    if (user) {
      navigate(RoutesConstant.login)      
    }

    bidShushi.verifyActivationAccountToken(token, ACCOUNT_ACTIVATION_VERIFY)
    .then((res) => {
    })
    .catch((err) => {
      api.error({
  ...
  const onSubmitHandler = (e) => {
    ...
    if (
      ...
      bidShushi.accountActivation(data,token).then(()=>{
        navigate(RoutesConstant.accountActivationSuccess)
      }).catch(err=>{
        api.error({
          message: `Failed`,
          description: 'Error in activating the account.',
          placement: 'topRight'
        })
      })
    } else {
        ...
        <div className="login-container ">
          ...
          <svg
            ...
            <form onSubmit={(e) => onSubmitHandler(e)}>
              ...
              <button type='submit' className="btn btn-secondary btn-submit w-100 mt-3 mb-3">Activate Account</button>
            </form>
          </div>
        </div>
      </main>
