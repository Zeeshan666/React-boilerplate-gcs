...
import { RESET_PASSWORD_VERIFY, USER_PASSWORD_REGEX } from '../../Common/index';
...
const Index = () => {
  ...
  useEffect(() => {
    document.title = "BidSushi | Reset Password";
    if (user) {
      navigate(RoutesConstant.login)      
    }
    bidShushi.verifyResetPasswordToken(token, RESET_PASSWORD_VERIFY)
    .then((res) => {
    // console.log(res)
    })
    .catch((err) => {
      ...
      navigate(`${RoutesConstant.login}`+'?message=Token Expired')
    });
    return () => {
      document.title = "BidSushi";
    }
  }, [])
  ...
  const onChangeHandler = (e) => {
    ...
    if (name === "password") {
      ...
      setError((preState) => {
        return {
          ...preState,
          confirmPassword: value ? false : true,
        };
      });
    }
  };
  ...
  const checkValidPassword = () => {
    if (password && password.length > 0) {
      if (
        password.length < 8 ||
        !password.match(USER_PASSWORD_REGEX)
      ) {
        setError((preState) => {
          return { ...preState, password: true };
        });
      } else {
        setError((preState) => {
          return { ...preState, password: false };
        });
      }
    }
  ...
  const checkValidConfirmPassword = () => {
    if (confirmPassword && confirmPassword.length > 0) {
      if (confirmPassword !== password) {
        setError((preState) => {
          return { ...preState, confirmPassword: true };
        });
      } else {
        setError((preState) => {
          return { ...preState, confirmPassword: false };
        });
      }
    }
  };
  ...
  const onSubmitHandler = (e) => {
    ...
    if (
      ...
      bidShushi.updatePassword(data,token).then(()=>{
        api.success({
          message: `Success`,
          description: 'Password has been updated successfully.',
          placement: 'topRight'
        })
        navigate(RoutesConstant.updatePasswordSuccess)
      }).catch(err=>{
        api.error({
          message: `Failed`,
          ...
          <svg
            ...
            <form onSubmit={(e) => onSubmitHandler(e)}>
              <div className="form-group">
                <label>Password <i className="float-end"><img alt='user' src="/images/key-icon.png" /> </i> </label>
                <input
                  type="password"
                  ...
                  onKeyDown={checkValidConfirmPassword}
                />
                {password && error.password ? (
                  <span className="text-danger text-error">
                    Password should be at least 8 characters long and should have at least one capital letter, one special character and one number.
                  </span>
                ) : null}
                {!password && error.password ? (
                  <span className="text-danger text-error">
              ...
              <div className="form-group">
                <label>Confirm Password <i className="float-end"><img alt='password' src="/images/key-icon.png" /> </i></label>
                <input
                  type="password"
                  className="form-control"
              ...
              <Link to={RoutesConstant.login} className="small">Back to login?</Link>
            </div> */}
              <button type='submit' className="btn btn-secondary btn-submit w-100 mt-3 mb-3">Reset Password</button>
            </form>
          </div>
        </div>
      </main>
