...
const Index = () => {
	...
	const searchParams = new URLSearchParams(location.search); //to get the query params
	const message = searchParams.get("message"); //to get the query starting with ?message

	const [api, contextHolder] = notification.useNotification(); //Notifications

	const navigate = useNavigate(); //to navigate to other pages

	const { dispatch, user } = useAuthContext(); //to get the context

	const [email, setEmail] = useState(""); // manage the state of email
	const [password, setPassword] = useState(""); // manage the state of password
	const [rememberMe, setRememberMe] = useState(false); // manage the state of remember me
	const [error, setError] = useState({
		// manage the state of error
		errorEmail: false,
		errorPassword: false,
	});
	...
	//Retrieve password and email if remember me is checked
	useEffect(() => {
	...
	//function to validate the email and password against pre-set regex
	...
	const postData = () => {
		if (
			...
			// error checking for email and password that failed above in errorChecking function
